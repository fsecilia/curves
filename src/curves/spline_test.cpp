...
TEST(spline_coefs, synchronous_as_sensitivity) {
  auto sensitivity =
      SynchronousCurve{0.433012701892L, 17.3205080757L, 5.33L, 8.3L, 0.5L};
...
TEST(spline_set, synchronous_as_transfer) {
  const auto sync_speed = 8.3L;
  auto sensitivity = SynchronousCurve{1.0, 10.0L, 1.0L, sync_speed, 0.5L};

  std::cout << sensitivity(0.0).df_dx << std::endl;

  const auto x_max = 128.0L;
  auto spline_set = generate_transfer_splines(sensitivity, sync_speed, x_max);
...
#if 0
    std::cout << "x_float: " << x_float << " expected: " << expected
              << " actual: " << actual << "\n"
              << std::endl;
#endif
