...
static inline u64 curves_fixed_isqrt(u64 x, unsigned int frac_bits,
				     unsigned int output_frac_bits)
{
	// isqrt magic constants
	//
	// To determine the initial NR guess, we linearly approximate 1/sqrt(x)
	// with the line `1.801688366500053410479 - 0.828427124746190097603*x`.
	// The actual constants are in Q2.62, so they're scaled by 2^62 and
	// rounded. The values are generated by sollya. For more information,
	// see src/curves/tools/isqrt_initial_guess.sollya.
	u64 GUESS_C0_Q62 = 8308821049351575724ULL;
	u64 GUESS_C1_Q62 = 3820445788478006404ULL;
	...
	y = GUESS_C0_Q62 -
	    (u64)(((u128)x_norm * GUESS_C1_Q62) >> X_NORM_FRAC_BITS);

	// Newton-Raphson: y[n + 1] = y[n](3 - x*y[n]^2)
	// 4 is chosen based on how many gamut tests needed tolerances.
	for (int i = 0; i < 4; ++i) {
