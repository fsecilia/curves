...
// ----------------------------------------------------------------------------
// __curves_fixed_isqrt_initial_guess()
// ----------------------------------------------------------------------------

struct IsqrtInitialGuessParam {
  s64 value;
  unsigned int frac_bits;
  unsigned int output_frac_bits;
  s64 expected_result;

  friend auto operator<<(std::ostream& out, const IsqrtInitialGuessParam& src)
      -> std::ostream& {
...
struct IsqrtInitialGuessTest : TestWithParam<IsqrtInitialGuessParam> {};

TEST_P(IsqrtInitialGuessTest, expected_result) {
  const auto actual_result = __curves_fixed_isqrt_initial_guess(
      GetParam().value, GetParam().frac_bits, GetParam().output_frac_bits);

  ASSERT_EQ(GetParam().expected_result, actual_result);
}

const IsqrtInitialGuessParam isqrt_initial_guess_smoke_test[] = {
    // Standard Case: sqrt(4.0) -> guess 2.0
    // Q20 -> Q20. log2(4) = 2. 2/2 = 1. Result 2^1.
    {1LL << 22, 20, 20, 1LL << 21},

    // Negative Log Case: sqrt(0.25) -> guess 0.5
    // Q20 -> Q20. log2(0.25) = -2. -2/2 = -1. Result 2^-1.
    {1LL << 18, 20, 20, 1LL << 19},

    // Odd Power (Floor Behavior): sqrt(8.0) -> guess 2.0
    // Q20 -> Q20. log2(8) = 3. 3/2 = 1 (integer trunc). Result 2^1.
    // (Actual sqrt(8) is ~2.82, so 2.0 is the correct power-of-2 floor).
    {1LL << 23, 20, 20, 1LL << 21},

    // Precision Upscale: sqrt(4.0)
    // Q10 input -> Q20 output.
    // Input: 4.0 in Q10 (1<<12). Output: 2.0 in Q20 (1<<21).
    {1LL << 12, 10, 20, 1LL << 21},

    // Precision Downscale: sqrt(16.0) -> guess 4.0
    // Q20 input -> Q10 output.
    // Input: 16.0 in Q20 (1<<24). Output: 4.0 in Q10 (1<<12).
    {1LL << 24, 20, 10, 1LL << 12},

    // Large Value: sqrt(2^30) -> guess 2^15
    // Q20 -> Q20.
    {1LL << 50, 20, 20, 1LL << 35},
};
INSTANTIATE_TEST_SUITE_P(smoke_tests, IsqrtInitialGuessTest,
                         ValuesIn(isqrt_initial_guess_smoke_test));
