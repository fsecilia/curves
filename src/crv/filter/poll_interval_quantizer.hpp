...
///
/// Each delivered report consumes at least one tick. One observation may represent multiple elapsed ticks because
/// zero-displacement reports may be absent.
///
...
///
/// This type is currently participating in a temporary experiment to write timestamps back to the input stream so a
/// downstream mouse acceleration driver sees the quantized timestamps. This will only remain live until this driver
/// pipeline proper comes online. Then writing timestamps back won't be necessary and regions marked `dev-only absolute
/// timestamp projection` can be removed.
class poll_interval_quantizer_t
{
public:
    using timestamp_t = fixed_t<uint64_t, 0>;

    // Q32.32 provides sub-nanosecond resolution over the required period range
    ...
    struct interval_t
    {
        ...
        // true when no forward tick was inferred and the result was clamped to one
    ...
    /// after initialization, report_period applies to (previous_timestamp, timestamp]
    [[nodiscard]] constexpr auto observe(timestamp_t timestamp, period_t report_period) noexcept -> interval_t
    {
        // any reachable corrected interval must fit tick_count_t
        assert(report_period >= period_t{1});

        if (!initialized_) [[unlikely]] { return initialize(timestamp, report_period); }
        if (timestamp < previous_timestamp_) [[unlikely]] { return reanchor(timestamp, report_period); }
        return update(timestamp, report_period);
    }
...
private:
    ...
    constexpr auto initialize(timestamp_t timestamp, period_t report_period) noexcept -> interval_t
    {
    ...
    constexpr auto reanchor(timestamp_t timestamp, period_t report_period) noexcept -> interval_t
    {
        // reanchor raw time while preserving the logical timeline
        previous_timestamp_ = timestamp;
        residual_ = {};
    ...
    constexpr auto update(timestamp_t timestamp, period_t report_period) noexcept -> interval_t
    {
    ...
    {
        if (corrected_interval <= residual_t{}) return {.elapsed_ticks = 1, .minimum_tick_forced = true};
        ...
        if (rounded_ticks.value == 0) return {.elapsed_ticks = 1, .minimum_tick_forced = true};

        return {.elapsed_ticks = rounded_ticks.value, .minimum_tick_forced = false};
    }
