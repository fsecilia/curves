...
struct refinement_pool_seeder_t
{
    ...
    constexpr auto operator()(typestate_t&& state, auto const& sample_transfer,
        critical_points_t const& critical_points) const -> typename typestate_t::next_t
    {
        ...
        auto left_x = x_t{0};
        auto left_sample = sample_transfer(jet_t{from_fixed<scalar_t>(left_x), scalar_t{1}});

        for (auto const right_x : critical_points)
        {
            left_sample = seed_interval(sample_transfer, left_sample, left_x, right_x, refinement_pool);
            left_x = right_x;
        }

        seed_interval(sample_transfer, left_sample, left_x, domain_end, refinement_pool);

        return typename typestate_t::next_t{workspace};
    }

private:
    constexpr auto seed_interval(auto const& sample_transfer, function_sample_t const& left_sample, x_t left_x,
        x_t right_x, auto& refinement_pool) const -> function_sample_t
    {
        auto const subdomain = create_subdomain(sample_transfer, left_sample, left_x, right_x);
        refinement_pool.emplace(create_interval(sample_transfer, subdomain));
