...
struct target_t
{
    constexpr auto transfer(scalar_t x) const noexcept -> scalar_t { return x * 3; }
    constexpr auto transfer(jet_t x) const noexcept -> jet_t { return {x.f * 2, x.df * 16}; }
};

using sut_t = transfer_sampler_t<target_t>;
constexpr auto sut = sut_t{target_t{}};
