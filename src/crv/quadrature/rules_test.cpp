...
constexpr auto near(scalar_t a, scalar_t b, scalar_t tolerance = 1e-12) noexcept -> bool
{
    return abs(a - b) <= tolerance;
}

constexpr auto power(scalar_t base, int exponent) noexcept -> scalar_t
{
    auto result = scalar_t{1};
    for (auto i = 0; i < exponent; ++i) { result *= base; }
    return result;
}

constexpr auto rule = gauss_kronrod_t<scalar_t>{};
...
static_assert(near(rule.integrate(1.0, 1.0, [](scalar_t x) { return x * x; }), 0.0));

// negative bounds: f(x) = x^2, integral [-3, -1] = 26/3
static_assert(near(rule.integrate(-3.0, -1.0, [](scalar_t x) { return x * x; }), 26.0 / 3.0));

// large offset: f(x) = x^2, integral [1000, 1002] = 6012008/3
static_assert(near(rule.integrate(1000.0, 1002.0, [](scalar_t x) { return x * x; }), 6012008.0 / 3.0, 1e-6));

// tiny interval: f(x) = x^2, integral [0, 0.001] = 1e-9/3
static_assert(near(rule.integrate(0.0, 0.001, [](scalar_t x) { return x * x; }), 1e-9 / 3.0));

// odd function, symmetric bounds: f(x) = x^3, integral [-1, 1] = 0
static_assert(near(rule.integrate(-1.0, 1.0, [](scalar_t x) { return power(x, 3); }), 0.0));

// odd function, asymmetric bounds: f(x) = x^3, integral [1, 3] = 20
static_assert(near(rule.integrate(1.0, 3.0, [](scalar_t x) { return power(x, 3); }), 20.0));

// rational to transcendental, f(x) = 1 / x, integral [1, 2] = ln(2) ≈ 0.6931471805599453
static_assert(near(rule.integrate(1.0, 2.0, [](scalar_t x) { return 1.0 / x; }), 0.6931471805599453, 1e-5));

// rational to transcendental, f(x) = 1 / (1 + x^2), integral [0, 1] = arctan(1) = pi/4 ≈ 0.7853981633974483
static_assert(near(rule.integrate(0.0, 1.0, [](scalar_t x) { return 1.0 / (1.0 + x * x); }), 0.7853981633974483, 1e-5));
...
constexpr auto total_exact = rule.estimate(0.0, 1.0, [](scalar_t x) { return power(x, 13); });
...
constexpr auto k15_exact = rule.estimate(0.0, 1.0, [](scalar_t x) { return power(x, 23); });
...
TEST(quadrature_rules_test, endpoint_singularity_log)
{
    ...
    auto const actual = rule.integrate(0.0, 1.0, [](scalar_t x) { return std::log(x); });
...
TEST(quadrature_rules_test, endpoint_singularity_sqrt)
{
    ...
    auto const actual = rule.integrate(0.0, 1.0, [](scalar_t x) { return 1.0 / std::sqrt(x); });
    EXPECT_NEAR(actual, expected, 5e-2);
}
...
TEST(quadrature_rules_test, fractional_power_law)
{
    ...
    auto const actual = rule.estimate(0.0, 1.0, [](scalar_t x) { return std::pow(x, 1.5); });
...
TEST(quadrature_rules_test, smooth_transcendental_log)
{
    ...
    auto const actual = rule.estimate(1.0, 2.0, [](scalar_t x) { return std::log(x); });
...
TEST(quadrature_rules_test, oscillatory_sin)
{
    ...
    auto const actual = rule.estimate(0.0, std::numbers::pi, [](scalar_t x) { return std::sin(10.0 * x); });
