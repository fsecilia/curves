...
template <typename t_cutoff_interval_t>
    requires(is_signed_v<typename t_cutoff_interval_t::value_t>)
struct cutoff_interval_calculator_t
{
    ...
    constexpr auto operator()(cutoff_rate_t cutoff_rate, dt_ns_t dt_ns) const noexcept -> cutoff_interval_t
    {
        ...
        static constexpr auto product_ceiling = product_t::template convert<shifter_t<rounding_modes::shr::truncate>{}>(
            cutoff_interval_t::maximum_finite);
        ...
        static_assert(cutoff_interval_value_t::convert(product_ceiling) <= cutoff_interval_t::maximum_finite);
        ...
        // a positive representable cutoff rate cannot underflow to zero here.
        auto const value
        ...
        assert(value <= cutoff_interval_t::maximum_finite);
