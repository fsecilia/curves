...
// 3/16 * 5 = 15/16.
constexpr auto ordinary = sut(cutoff_rate_t::literal(3), dt_ns_t{5});
static_assert(!ordinary.is_limit);
static_assert(ordinary.value == cutoff_interval_value_t::literal(15));

constexpr auto smallest = sut(cutoff_rate_t::literal(1), dt_ns_t{1});
static_assert(!smallest.is_limit);
static_assert(smallest.value == cutoff_interval_value_t::literal(1));

constexpr auto maximum_finite = sut(cutoff_rate_t::literal(cutoff_interval_t::maximum_finite.value), dt_ns_t{1});
...
constexpr auto first_above_maximum_finite
    = sut(cutoff_rate_t::literal(cutoff_interval_t::maximum_finite.value + 1), dt_ns_t{1});
...
using sut_fine_t = cutoff_interval_calculator_t<one_euro::cutoff_interval_t<fine_cutoff_interval_value_t>>;
constexpr auto sut_fine = sut_fine_t{};

constexpr auto truncated_ceiling = sut_fine(coarse_cutoff_rate_t::literal(27), dt_ns_t{1});
static_assert(!truncated_ceiling.is_limit);
static_assert(truncated_ceiling.value == fine_cutoff_interval_value_t::literal(108));

constexpr auto first_above_truncated_ceiling = sut_fine(coarse_cutoff_rate_t::literal(28), dt_ns_t{1});
