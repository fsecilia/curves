...
/// \file
/// \brief unit and integration tests for the fixed-point 1-euro filter
...
struct pipeline_filters_one_euro_filter_integration_test_t : Test
{
    ...
    static constexpr auto cutoff_ceiling = sut_t::cutoff_step_calculator_t::cutoff_step_clamp_t::cutoff_ceiling;
    ...
    static constexpr auto make_literal_params(
        cutoff_step_t omega_derivative, cutoff_step_t omega_min, cutoff_step_t beta) -> sut_t::params_t
    {
    ...
    {
        return {
            .omega_derivative = to_fixed<cutoff_step_t>(two_pi_per_ns * f_c_derivative),
            .omega_min = to_fixed<cutoff_step_t>(two_pi_per_ns * f_c_min),
            .beta = to_fixed<cutoff_step_t>(two_pi_per_ns * beta),
        };
    }
...
{
    using cutoff_step_t = pipeline_filters_one_euro_filter_integration_test_t::cutoff_step_t;
    using alpha_t = pipeline_filters_one_euro_filter_integration_test_t::alpha_t;
    using clamp_t = cutoff_step_clamp_t<cutoff_step_t>;

    using wide_cutoff_step_t = fixed::widened_t<cutoff_step_t>;
    constexpr auto wide_ceiling = wide_cutoff_step_t::convert(clamp_t::cutoff_ceiling);
    constexpr auto clamped_step = clamp_t{}(wide_ceiling);
    constexpr auto alpha = alpha_map_t<alpha_t>{}(clamped_step);
    return alpha < max<alpha_t>();
}
static_assert(signal_cutoff_ceiling_maps_below_one());

constexpr auto real_graph_is_constexpr_and_huge_dt_uses_signal_ceiling() noexcept -> bool
{
    ...
    auto sut = fixture_t::sut_t{fixture_t::make_literal_params(
        fixture_t::cutoff_step_t{}, fixture_t::cutoff_step_t::literal(1), fixture_t::cutoff_step_t{})};
...
static_assert(real_graph_is_constexpr_and_huge_dt_uses_signal_ceiling());

TEST_F(pipeline_filters_one_euro_filter_integration_test_t,
    long_idle_clears_stale_derivative_state_without_a_signal_style_step_clamp)
{
    auto sut = sut_t{make_literal_params(cutoff_step_t{1}, cutoff_step_t{}, cutoff_step_t{})};
...
TEST_F(pipeline_filters_one_euro_filter_integration_test_t, converges_to_the_expected_rne_stall_residual)
{
    auto sut = sut_t{make_params(10.0, 1.0, 0.0)};
...
TEST_F(pipeline_filters_one_euro_filter_integration_test_t,
    beta_zero_step_response_matches_the_analytic_first_order_response)
{
...
TEST_F(pipeline_filters_one_euro_filter_integration_test_t, larger_dt_produces_a_larger_single_sample_response)
{
    auto const params = make_params(10.0, 1.0, 0.0);
    constexpr auto target = x_t{1000};
...
TEST_F(pipeline_filters_one_euro_filter_integration_test_t,
    constant_input_slope_matches_the_analytic_derivative_response_across_sample_rates)
{
