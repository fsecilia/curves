...
template <typename t_narrow_t> struct division_long_divider_test_t : ::testing::Test
{
    using narrow_t = t_narrow_t;
    using wide_t   = wider_t<narrow_t>;
    using result_t = result_t<wide_t, narrow_t>;

    static constexpr auto wide_bit_count = sizeof(wide_t) * CHAR_BIT;
    static constexpr auto max_wide       = ~wide_t{0};
    static constexpr auto max_narrow     = ~narrow_t{0};
    static constexpr auto shift          = wide_bit_count / 2;
    static constexpr auto low_mask       = (wide_t{1} << shift) - 1;

    struct hardware_divider_t
    {
        auto verify_preconditions(wide_t dividend, narrow_t divisor) const noexcept -> void
        {
        ...
        auto operator()(wide_t dividend, narrow_t divisor) const noexcept -> result_t
        {
            ...
            return result_t{.quotient  = int_cast<wide_t>(dividend / divisor),
                            .remainder = int_cast<narrow_t>(dividend % divisor)};
        }
    };

    using sut_t = long_divider_t<wide_t, narrow_t, hardware_divider_t>;

    sut_t sut{hardware_divider_t{}};

    struct full_width_generator_t
    {
        auto dividend(auto& rng) noexcept -> wide_t
        {
            return (static_cast<wide_t>(rng()) << shift) | static_cast<wide_t>(rng());
        }

        auto divisor(auto& rng) noexcept -> wide_t { return static_cast<narrow_t>(rng()); }
    };

    struct random_length_generator_t : full_width_generator_t
    {
        std::uniform_int_distribution<int_t> bit_count_dist{0, wide_bit_count - 1};
        ...
        auto divisor(auto& rng) noexcept -> wide_t
        {
    ...
    auto fuzz(auto&& generate) const noexcept -> void
    {
        ...
        for (auto i = 0; i < 10000; ++i)
        {
            ...
            auto const expected
                = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};

            auto const actual = this->sut(dividend, divisor);
...
TYPED_TEST(division_long_divider_test_t, zero)
{
    ...
    auto const dividend = wide_t{0};
    auto const divisor  = narrow_t{1};
...
TYPED_TEST(division_long_divider_test_t, self_division)
{
    ...
    auto const dividend = wide_t{5000}; // arbitrary, > 1, fits in divisor_t
    auto const divisor  = int_cast<narrow_t>(dividend);
    auto const expected = result_t{.quotient = 1, .remainder = 0};
...
TYPED_TEST(division_long_divider_test_t, basic)
{
    ...
    auto const dividend = wide_t{6};
    auto const divisor  = narrow_t{3};
...
TYPED_TEST(division_long_divider_test_t, test_high_divisor_minus_one)
{
    ...
    auto const divisor  = narrow_t{5000};        // arbitrary
    auto const high     = int_cast<wide_t>(divisor - 1);
    auto const low      = TestFixture::low_mask; // maximize low half
    auto const dividend = (high << TestFixture::shift) | low;
    auto const expected = result_t{.quotient = low, .remainder = int_cast<narrow_t>(high)};
...
TYPED_TEST(division_long_divider_test_t, high_equals_divisor)
{
    ...
    auto const divisor  = narrow_t{5000}; // arbitrary
    auto const high     = wide_t{divisor};
    auto const low      = wide_t{TestFixture::low_mask};
    auto const dividend = (high << TestFixture::shift) | low;
    auto const expected = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};
...
TYPED_TEST(division_long_divider_test_t, max_divisor_max_remainder_carry)
{
    ...
    auto const divisor  = TestFixture::max_narrow;
    auto const high     = int_cast<wide_t>(divisor - 1);
...
TYPED_TEST(division_long_divider_test_t, empty_low_half)
{
    ...
    auto const divisor  = int_cast<narrow_t>(TestFixture::max_narrow);
    auto const high     = int_cast<wide_t>(divisor - 1);
    auto const low      = int_cast<wide_t>(0);
    auto const dividend = (high << TestFixture::shift) | low;
    auto const expected = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};
...
TYPED_TEST(division_long_divider_test_t, dividend_smaller_than_divisor)
{
    ...
    auto const divisor  = TestFixture::max_narrow;
    auto const dividend = int_cast<wide_t>(divisor - 1);
    auto const expected = result_t{.quotient = 0, .remainder = int_cast<narrow_t>(dividend)};
...
TYPED_TEST(division_long_divider_test_t, only_msb_set)
{
    ...
    auto const dividend = wide_t{1} << (TestFixture::wide_bit_count - 1);
...
// tests maximum possible wide dividend divided by 1
//
//     [~0][~0]1
//
...
TYPED_TEST(division_long_divider_test_t, max_capacity)
{
    ...
    auto const dividend = TestFixture::max_wide;
    auto const divisor  = narrow_t{1};
...
TYPED_TEST(division_long_divider_test_t, power_of_two_divisor)
{
    ...
    auto const divisor  = narrow_t{1} << (TestFixture::shift - 1);
    auto const dividend = TestFixture::max_wide;
...
TYPED_TEST(division_long_divider_test_t, zero_remainder_carry)
{
    ...
    auto const divisor  = narrow_t{256};
    auto const high     = int_cast<wide_t>(divisor * 3);
    auto const low      = int_cast<wide_t>(divisor + 1);
    auto const dividend = (high << TestFixture::shift) | low;
    auto const expected = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};
...
TYPED_TEST(division_long_divider_test_t, alternating_bits_even)
{
    ...
    // 0xAAAA...AAAA
    auto const dividend = TestFixture::max_wide / 3 * 2;
    auto const divisor  = narrow_t{7};
    auto const expected = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};
...
TYPED_TEST(division_long_divider_test_t, alternating_bits_odd)
{
    ...
    // 0x5555...5555
    auto const dividend = TestFixture::max_wide / 3;
    auto const divisor  = narrow_t{7};
    auto const expected = result_t{.quotient = dividend / divisor, .remainder = int_cast<narrow_t>(dividend % divisor)};
