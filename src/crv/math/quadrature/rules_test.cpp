...
constexpr auto rule = gauss_kronrod_t<15>{};

// constant: f(x) = 5, integral [0, 2] = 10
static_assert(near(rule(0.0, 2.0, [](real_t) { return 5.0; }).value, 10.0));

// reversed bounds: f(x) = x^2, integral [2, 0] = -8/3
static_assert(near(rule(2.0, 0.0, [](real_t x) { return x * x; }).value, -8.0 / 3.0));

// zero-width: f(x) = x^2, integral [1, 1] = 0
static_assert(near(rule(1.0, 1.0, [](real_t x) { return x * x; }).value, 0.0));

// negative bounds: f(x) = x^2, integral [-3, -1] = 26/3
static_assert(near(rule(-3.0, -1.0, [](real_t x) { return x * x; }).value, 26.0 / 3.0));

// large offset: f(x) = x^2, integral [1000, 1002] = 6012008/3
static_assert(near(rule(1000.0, 1002.0, [](real_t x) { return x * x; }).value, 6012008.0 / 3.0, 1e-6));

// tiny interval: f(x) = x^2, integral [0, 0.001] = 1e-9/3
static_assert(near(rule(0.0, 0.001, [](real_t x) { return x * x; }).value, 1e-9 / 3.0));

// odd function, symmetric bounds: f(x) = x^3, integral [-1, 1] = 0
static_assert(near(rule(-1.0, 1.0, [](real_t x) { return power(x, 3); }).value, 0.0));

// odd function, asymmetric bounds: f(x) = x^3, integral [1, 3] = 20
static_assert(near(rule(1.0, 3.0, [](real_t x) { return power(x, 3); }).value, 20.0));

// rational to transcendental, f(x) = 1 / x, integral [1, 2] = ln(2) ≈ 0.6931471805599453
static_assert(near(rule(1.0, 2.0, [](real_t x) { return 1.0 / x; }).value, 0.6931471805599453, 1e-5));

// rational to transcendental, f(x) = 1 / (1 + x^2), integral [0, 1] = arctan(1) = pi/4 ≈ 0.7853981633974483
static_assert(near(rule(0.0, 1.0, [](real_t x) { return 1.0 / (1.0 + x * x); }).value, 0.7853981633974483, 1e-5));
...
constexpr auto total_exact = rule(0.0, 1.0, [](real_t x) { return power(x, 13); });
static_assert(near(total_exact.value, 1.0 / 14.0));
...
constexpr auto kronrod_exact = rule(0.0, 1.0, [](real_t x) { return power(x, 23); });
static_assert(near(kronrod_exact.value, 1.0 / 24.0));
...
constexpr auto demonstrated_failure = rule(0.0, 1.0, [](real_t x) { return power(x, 40); });
static_assert(abs(demonstrated_failure.value - (1.0 / 41.0)) > std::numeric_limits<real_t>::epsilon());
...
TEST(quadrature_rules_test, endpoint_singularity_log)
{
    ...
    auto const actual       = rule(0.0, 1.0, [](real_t x) { return std::log(x); });
    EXPECT_NEAR(actual.value, expected, 5e-2);
}
...
TEST(quadrature_rules_test, endpoint_singularity_sqrt)
{
    ...
    auto const actual       = rule(0.0, 1.0, [](real_t x) { return 1.0 / std::sqrt(x); });
    EXPECT_NEAR(actual.value, expected, 5e-2);
}
...
TEST(quadrature_rules_test, fractional_power_law)
{
    ...
    auto const actual       = rule(0.0, 1.0, [](real_t x) { return std::pow(x, 1.5); });

    EXPECT_NEAR(actual.value, expected, 1e-7);
    EXPECT_GT(actual.error, 0.0);
    EXPECT_LT(actual.error, 1e-5);
    EXPECT_LE(crv::abs(expected - actual.value), actual.error);
}
...
TEST(quadrature_rules_test, smooth_transcendental_log)
{
    ...
    auto const actual   = rule(1.0, 2.0, [](real_t x) { return std::log(x); });

    EXPECT_NEAR(actual.value, expected, 1e-14);
    EXPECT_LE(crv::abs(expected - actual.value), actual.error);
}
...
TEST(quadrature_rules_test, oscillatory_sin)
{
    ...
    auto const actual       = rule(0.0, std::numbers::pi, [](real_t x) { return std::sin(10.0 * x); });
