...
static_assert(refinement.left.right == 3.0);
static_assert(refinement.left.tolerance == initial_tolerance * 0.5);
...
static_assert(refinement.right.right == 6.0);
static_assert(refinement.right.tolerance == initial_tolerance * 0.5);
...
constexpr auto nested_refinement = sut.refine(refinement.left);

// depth increments off the child, not the root
static_assert(nested_refinement.left.depth == 2);
static_assert(nested_refinement.right.depth == 2);

// tolerance halves again, so 1/4 of the root tolerance
static_assert(nested_refinement.left.tolerance == initial_tolerance * 0.25);
static_assert(nested_refinement.right.tolerance == initial_tolerance * 0.25);

// left child of [0, 3] is [0, 1.5]; right child is [1.5, 3]
static_assert(nested_refinement.left.left == 0.0);
static_assert(nested_refinement.left.right == 1.5);
static_assert(nested_refinement.right.left == 1.5);
static_assert(nested_refinement.right.right == 3.0);
...
constexpr auto zero_width_refinement = sut.refine(zero_width_parent);
...
constexpr auto reversed_refinement = sut.refine(reversed_parent);
