...
struct integral_cache_builder_test_t : Test
{
    using real_t        = float_t;
    using accumulator_t = real_t;

    struct integral_cache_t
    {
    ...
    using sut_t = integral_cache_builder_t<real_t, integral_cache_t, accumulator_t>;

    sut_t sut{};
};

TEST_F(integral_cache_builder_test_t, append_none)
{
    EXPECT_EQ((typename integral_cache_t::map_t{{0, 0}}), std::move(sut).finalize().map);
}

TEST_F(integral_cache_builder_test_t, append_one)
{
    sut.append(3.0, 5.0);
    EXPECT_EQ((typename integral_cache_t::map_t{{0, 0}, {3.0, 5.0}}), std::move(sut).finalize().map);
}

TEST_F(integral_cache_builder_test_t, append_many)
{
    sut.append(3.0, 5.0);
    sut.append(5.5, 2.3);
    sut.append(11.6, 1.1);
    EXPECT_EQ((typename integral_cache_t::map_t{{0, 0}, {3.0, 5.0}, {5.5, 5.0 + 2.3}, {11.6, 5.0 + 2.3 + 1.1}}),
              std::move(sut).finalize().map);
}

TEST_F(integral_cache_builder_test_t, nonmonotonic)
{
    EXPECT_DEBUG_DEATH(sut.append(0.0, -1.0), "monotonically increasing");
}

TEST_F(integral_cache_builder_test_t, monotonic_nonincreasing)
{
