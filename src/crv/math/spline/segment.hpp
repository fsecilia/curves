...
public:
    ...
    constexpr auto is_monotonic() const noexcept -> bool
    {
        ...
        auto const a = unpack_coeff0();
        auto const b = coeffs_[1];
        auto const c = coeffs_[2];

        // check the left endpoint: `p'(0) = C >= 0`
        if (c < coeff_t{0}) return false;

        // check the right endpoint: `p'(1) = 3A + 2B + C >= 0`
        if (3 * a + 2 * b + c < coeff_t{0}) return false;
        ...
        if (b <= -3 * a || coeff_t{0} <= b) return true;
