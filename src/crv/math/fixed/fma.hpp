...
struct fma_t
{
    ...
    static_assert(
        out_shift <= 0 || out_shift < actual_sum_width, "fma upscaling shift exceeds intermediate container width");
    static_assert(
        out_shift >= 0 || -out_shift < actual_sum_width, "fma downscaling shift exceeds intermediate container width");
