...
template <is_fixed out_t, bool saturate = false> struct fma_t
{
    template <is_fixed multipicand_t, is_fixed multiplier_t, is_fixed addend_t,
              typename rounding_mode_t = fixed::default_shr_rounding_mode_t>
    constexpr auto operator()(multipicand_t multiplicand, multiplier_t multiplier, addend_t addend,
                              rounding_mode_t rounding_mode = {}) noexcept -> out_t
    {
        ...
        static_assert(product_shift < sizeof(sum_value_t) * CHAR_BIT,
                      "fma radix alignment shift exceeds container width");
        static_assert(addend_shift < sizeof(sum_value_t) * CHAR_BIT,
                      "fma radix alignment shift exceeds container width");
