...
public:
    ...
    auto inspect(reflection::param_t<value_t, constraint_t>& param) -> void
    {
        value_t value;
    ...
    /// recurses inspector into nested section if found; does nothing if not present, reports error if not a section
    ...
    auto inspect_section(std::string_view name, section_inspector_t&& section_inspector) -> void
    {
        if (auto section_adapter = adapter_.get_section(name))
        {
            ...
            std::invoke(std::forward<section_inspector_t>(section_inspector), reader_t{*std::move(section_adapter)});
        }
    }
