...
{
public:
    constexpr input_value_array_view_t(
        void* values, std::size_t count, std::size_t capacity, uint64_t timestamp_ns) noexcept
        : values_{static_cast<std::byte*>(values)}, timestamp_ns_{timestamp_ns}, count_{count}, capacity_{capacity}
    {
    ...
    /// \returns timestamp of record, in nanoseconds
    constexpr auto timestamp_ns() const noexcept -> uint64_t { return timestamp_ns_; }

    /// \returns loads occupied record
    [[nodiscard]] constexpr auto load(std::size_t index) const noexcept -> input_value_t
    {
        assert(index < count_);
        ...
        return input_value_t{
            ...
            .code = static_cast<input_value_t::code_t>(result.code),
            .value = int_cast<input_value_t::value_t>(result.value),
        };
    }

    /// \returns replaces occupied record
    constexpr auto store(std::size_t index, input_value_t const& value) noexcept -> void
    {
        assert(index < count_);

        auto const result = crv_input_value_t{
            .type = static_cast<crv_input_u16_t>(value.type),
            .code = static_cast<crv_input_u16_t>(value.code),
            .value = int_cast<crv_input_s32_t>(value.value),
        };

        __builtin_memcpy(address(index), &result, sizeof(result));
    }

    /// appends x or y record to end of array, moving SYN_REPORT to accomodate
    constexpr auto append(input_value_t::code_rel_t code, input_value_t::value_t value) noexcept -> void
    {
        assert(count_ < capacity_);
        ++count_;

        // move SYN_REPORT right one slot
        store(count_ - 1, load(count_ - 2));
    ...
    /// appends x and y record, moving SYN_REPORT to accomodate
    constexpr auto append(input_value_t::value_t x, input_value_t::value_t y) noexcept -> void
    {
        assert(count_ < capacity_);
    ...
    /// erases an x or y record
    constexpr auto erase(std::size_t index) noexcept -> void
    {
        assert(count_ > 1);
    ...
    /// erases x and y record
    constexpr auto erase(std::size_t x_index, std::size_t y_index) noexcept -> void
    {
        assert(count_ > 2);
...
private:
    ...
    [[nodiscard]] constexpr auto address(this auto&& self, std::size_t index) noexcept -> void*
    {
        return self.values_ + index * stride;
    }
