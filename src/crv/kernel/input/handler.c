...
struct crv_input_handle_t
{
    struct input_handle input;
    struct crv_control_attachment_t control_attachment;
...
// capability and device policy match
...
{
    struct crv_input_handler_t* crv_handler = container_of(handler, struct crv_input_handler_t, input);
    struct crv_input_handle_t* crv_handle;
    ...
    error = crv_control_attach(crv_handler->control, &crv_handle->control_attachment, handle, handle->private);
...
{
    struct crv_input_handler_t* crv_handler = container_of(handle->handler, struct crv_input_handler_t, input);
    struct crv_input_handle_t* crv_handle = container_of(handle, struct crv_input_handle_t, input);
    struct input_dev* device = handle->dev;

    crv_control_detach(crv_handler->control, &crv_handle->control_attachment);
    input_close_device(handle);
    input_unregister_handle(handle);
...
int crv_input_handler_register(struct crv_input_handler_t* handler, struct crv_control_t* control)
{
    ...
    *handler = (struct crv_input_handler_t){
        .input = {
    ...
    error = input_register_handler(&handler->input);
    if (error) pr_err("failed to register input handler: %d\n", error);
...
{
    input_unregister_handler(&handler->input);
}
