...
constexpr auto matches(input_value_array_view_t const& actual, std::array<input_value_t, size> const& expected) noexcept
    -> bool
{
...
constexpr auto timestamp_ns = uint64_t{123'456'789};
...
struct input_value_array_view_test_t : Test
{
    ...
    {
        return input_value_array_view_t{
            storage_.data(),
            count,
            capacity,
            timestamp_ns,
...
TEST_F(input_value_array_view_test_t, store_preserves_timestamp)
{
    storage_[0] = syn_report();

    auto sut = make(1);
    sut.store(0, syn_report(1));

    EXPECT_EQ(sut.timestamp_ns(), timestamp_ns);
}
...
TEST_F(input_value_array_view_test_t, appends_x_before_syn_report)
{
    ...
    EXPECT_TRUE(matches(sut, std::array{rel_x(17), syn_report()}));
}
...
TEST_F(input_value_array_view_test_t, appends_y_before_syn_report)
{
    ...
    EXPECT_TRUE(matches(sut, std::array{rel_y(-17), syn_report()}));
}
...
TEST_F(input_value_array_view_test_t, appends_two_records_before_syn_report)
{
    ...
    EXPECT_TRUE(matches(sut, std::array{rel_x(17), rel_y(-29), syn_report()}));
}
...
TEST_F(input_value_array_view_test_t, single_append_preserves_existing_record_order)
{
    ...
    EXPECT_TRUE(matches(sut, std::array{event_a, event_b, rel_x(17), syn_report()}));
}
...
TEST_F(input_value_array_view_test_t, pair_append_preserves_existing_record_order)
{
    ...
    EXPECT_TRUE(matches(sut, std::array{event_a, event_b, rel_x(17), rel_y(-29), syn_report()}));
}
...
TEST_F(input_value_array_view_test_t, append_preserves_timestamp)
{
    storage_[0] = syn_report();
...
TEST_F(input_value_array_view_test_t, pair_append_preserves_timestamp)
{
    storage_[0] = syn_report();
...
TEST_F(input_value_array_view_test_t, erase_preserves_timestamp)
{
    storage_[0] = rel_y(11);
    storage_[1] = syn_report();
...
{
    EXPECT_DEBUG_DEATH(input_value_array_view_t(nullptr, 1, 1, timestamp_ns), "nullptr != values_");
}

TEST_F(input_value_array_view_test_t, constructor_aborts_on_zero_capacity)
{
    EXPECT_DEBUG_DEATH(input_value_array_view_t(storage_.data(), 1, 0, timestamp_ns), "0 < capacity_");
}

TEST_F(input_value_array_view_test_t, constructor_aborts_on_zero_count)
{
    EXPECT_DEBUG_DEATH(input_value_array_view_t(storage_.data(), 0, 1, timestamp_ns), "0 < count_");
}

TEST_F(input_value_array_view_test_t, constructor_aborts_when_count_exceeds_capacity)
{
    EXPECT_DEBUG_DEATH(input_value_array_view_t(storage_.data(), 3, 2, timestamp_ns), "count_ <= capacity_");
}
